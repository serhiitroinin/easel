...
export const CLAUDE_ENGINE = "easel:claude";
export const CODEX_ENGINE = "easel:codex";
...
function linkCodexAuth(privateHome: string, accountHome: string): void {
  ...
  if (found?.isFile()) {
    ...
    if (!current || found.mtimeMs > current.mtimeMs) {
      const staged = join(accountHome, `.easel-auth-${process.pid}`);
...
/**
 * Both adapters drop tool output by default. The application tools are the only
 * tools here and their output is written by this application, so it is kept:
 * without it a refused call reaches the interface with no reason.
 */
function keepApplicationToolOutput(prefix: string) {
  return (tool: unknown, output: string): string => {
    const name = (tool as { name?: unknown }).name;
    const owned = typeof name === "string" && (name.startsWith(prefix) || !name.includes("__"));
    return owned ? output.slice(0, TOOL_OUTPUT_LIMIT) : "";
  };
}

export function createEngines(options: EngineOptions): HarnessAdapter[] {
  mkdirSync(options.workspace, { recursive: true, mode: 0o700 });
  const toolPrefix = "mcp__reins__";
  const events = { redactToolOutput: keepApplicationToolOutput(toolPrefix) };
